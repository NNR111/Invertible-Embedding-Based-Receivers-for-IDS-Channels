...
from ids_receiver.config import PAD_VALUE, EMBED_DIM, GRU_HIDDEN, GRU_LAYERS, MSG_LEN, CC_K, DROPOUT
...
class NBMHeadNoEmbed(nn.Module):
    def __init__(self, in_dim: int = EMBED_DIM, code_bits: int = 2 * (MSG_LEN + CC_K - 1), hidden: int = GRU_HIDDEN):
        super().__init__()
        self.code_bits = code_bits
        self.gru = nn.GRU(in_dim, hidden, batch_first=True, bidirectional=True, num_layers=GRU_LAYERS, dropout=DROPOUT if GRU_LAYERS > 1 else 0.0)
...
class BiGRUDecoder(nn.Module):
    def __init__(self, code_ctx_dim: int = GRU_HIDDEN * 2, hidden: int = GRU_HIDDEN, layers: int = GRU_LAYERS, msg_len: int = MSG_LEN):
        ...
        self.gru = nn.GRU(hidden, hidden, batch_first=True, bidirectional=True, num_layers=layers, dropout=DROPOUT if layers > 1 else 0.0)
