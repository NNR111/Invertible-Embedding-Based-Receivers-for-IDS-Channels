...
def main():
    ...
    ap.add_argument('--save_dir', type=str, default='runs_noembed/decoder')
    ...
    val_ds = SyntheticIDSDataset(args.val_samples, args.p_ins, args.p_del, args.p_sub_train_min, args.p_sub_train_max,
                                 use_marker=bool(args.use_marker), marker=marker, num_blocks=args.num_blocks, seed=args.seed + 56561)
    ...
    model.front.load_state_dict(nckpt['front_state'])
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f'[decoder_noembed] epoch {epoch:03d} train_loss={tr_loss:.6f} train_BER={tr_ber:.6f} train_BLER={tr_bler:.6f} val_loss={va_loss:.6f} val_BER={va_ber:.6f} val_BLER={va_bler:.6f}')
        ckpt = {'front_state': model.front.state_dict(), 'nbm_state': model.nbm.state_dict(), 'decoder_state': model.decoder.state_dict(), 'args': vars(args), 'epoch': epoch}
