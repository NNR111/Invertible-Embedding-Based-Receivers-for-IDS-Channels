...
def load_init_ckpt(model, init_ckpt: str):
    ...
    if "local_head_state" in ckpt:
        try:
            model.local_head.load_state_dict(ckpt["local_head_state"], strict=False)
        except Exception:
            pass
...
def main():
    ...
    # always load embedding checkpoint first
    ...
    if args.resume_ckpt is not None:
        ...
        if "nbm_state" in rckpt:
            model.nbm.load_state_dict(rckpt["nbm_state"], strict=False)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print(
            f"[nbm_embed] epoch {epoch:03d} "
            f"train_loss={tr_loss:.6f} train_codeBER={tr_cber:.6f} "
            f"val_loss={va_loss:.6f} val_codeBER={va_cber:.6f}"
        )
