...
def main():
    ...
    ap.add_argument('--save_dir', type=str, default='runs_noembed/nbm')
    ...
    val_ds = SyntheticIDSDataset(args.val_samples, args.p_ins, args.p_del, args.p_sub_train_min, args.p_sub_train_max,
                                 use_marker=bool(args.use_marker), marker=marker, num_blocks=args.num_blocks, seed=args.seed + 45451)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f'[nbm_noembed] epoch {epoch:03d} train_loss={tr_loss:.6f} train_codeBER={tr_cber:.6f} val_loss={va_loss:.6f} val_codeBER={va_cber:.6f}')
        ckpt = {'front_state': model.front.state_dict(), 'nbm_state': model.nbm.state_dict(), 'args': vars(args), 'epoch': epoch}
