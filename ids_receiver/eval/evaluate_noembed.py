...
def eval_one(model, n_trials, batch_size, p_ins, p_del, p_sub, device, seed, use_marker, marker, num_blocks):
    ds = SyntheticIDSDataset(n_trials, p_ins, p_del, p_sub, p_sub, use_marker=use_marker, marker=marker, num_blocks=num_blocks, seed=seed)
...
def main():
    ...
    ap.add_argument('--p_ins', type=float, default=0.0)
    ...
    ap.add_argument('--marker', type=str, default='0,3,0,3')
    ap.add_argument('--num_blocks', type=int, default=5)
    ap.add_argument('--out_csv', type=str, default='runs_noembed/eval_noembed.csv')
    args = ap.parse_args()
    ...
    model.front.load_state_dict(ckpt['front_state'])
    ...
    for p_str in args.p_sub_list.split(','):
        p_sub = float(p_str)
        ber, bler = eval_one(model, args.n_trials, args.batch_size, args.p_ins, args.p_del, p_sub, device,
                             args.seed + int(p_sub * 10000), bool(args.use_marker), marker, args.num_blocks)
