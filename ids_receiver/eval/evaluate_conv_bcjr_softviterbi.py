...
from ids_receiver.receivers.bcjr_soft_viterbi_receiver import decode_one_bcjr_softviterbi


def evaluate_one(p_ins: float,
                 p_del: float,
                 p_sub: float,
                 n_trials: int,
                 use_marker: bool,
    ...
    for _ in tqdm(range(n_trials), leave=False):
        ...
        out = decode_one_bcjr_softviterbi(
            msg_bits=msg,
            p_ins=p_ins,
            p_del=p_del,
            p_sub=p_sub,
...
def main():
    ...
    ap.add_argument('--out_csv', type=str, default='runs_bcjr_viterbi/eval_conv_bcjr_softviterbi.csv')
    args = ap.parse_args()
    ...
    for i, p_sub in enumerate(parse_p_sub_list(args.p_sub_list)):
        ...
        rows.append({'p_ins': args.p_ins, 'p_del': args.p_del, 'p_sub': p_sub, 'BER': ber, 'BLER': bler})
        print(f'p_ins={args.p_ins:.4f} p_del={args.p_del:.4f} p_sub={p_sub:.4f} BER={ber:.6f} BLER={bler:.6f}')

    out_dir = os.path.dirname(args.out_csv)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
    with open(args.out_csv, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['p_ins', 'p_del', 'p_sub', 'BER', 'BLER'])
        writer.writeheader()
