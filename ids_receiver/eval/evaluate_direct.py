...
def evaluate_one_setting(
    ...
    # Synthetic dataset generated directly in Python
    ...
    with torch.no_grad():
        for batch in tqdm(loader, leave=False, desc=f"p_sub={p_sub:.4f}"):
            # Model only sees the noisy channel output at inference
            noisy_syms = batch["noisy_syms"].to(device)
            noisy_lens = batch["noisy_lens"].to(device)

            # Ground-truth information bits, used only for evaluation
...
def main():
    ...
    ap.add_argument("--out_csv", type=str, default="runs_embed/eval_direct_python.csv")
