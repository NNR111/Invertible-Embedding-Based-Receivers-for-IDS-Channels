...
def convenc(msg_bits: Sequence[int], generators_octal: Sequence[int], constraint_length: int) -> np.ndarray:
...
def build_trellis(generators_octal: Sequence[int], constraint_length: int) -> Trellis:
    taps = [octal_to_taps(g, constraint_length) for g in generators_octal]
    ...
    for s in range(num_states):
        mem_bits = np.array([(s >> (memory - 1 - i)) & 1 for i in range(memory)], dtype=np.int64)
        for inp in (0, 1):
            full_reg = np.concatenate(([inp], mem_bits))
            out = np.array([int(np.sum(full_reg * tap) % 2) for tap in taps], dtype=np.int64)
    ...
    return Trellis(next_state=next_state, output_bits=output_bits, output_sym=output_sym)
...
def soft_input_viterbi(
    ...
    n_out = len(generators_octal)
    if llr.size % n_out != 0:
        raise ValueError("llr_vec length must be divisible by number of output bits per trellis step")
    ...
    for t in range(num_steps):
        for s in range(num_states):
            ...
            for inp in (0, 1):
                ...
                branch = sum(bit_cost_from_llr(rx[t, j], int(out_bits[j])) for j in range(n_out))
                cand = pm + branch
...
def _log_add_lookup(cur: float, cand: float, log_map_vec: np.ndarray, delta_step: float) -> float:
    t = abs(cand - cur)
...
def FB_decode(
        ...
        for n in range(1, R + 1):
            for l in range(l_max + 1):
                for b in (0, 1):
                    ...
                    log_alf[k, n + no] = _log_add_lookup(log_alf[k, n + no], calf, log_map_vec, delta_step)
        ...
        for n in range(R - 1, -1, -1):
            for l in range(l_max + 1):
                for b in (0, 1):
                    ...
                    log_bet[k, n + no] = _log_add_lookup(log_bet[k, n + no], cbet, log_map_vec, delta_step)
                ...
                for b in (0, 1):
                    for n in range(0, upper_val + 1):
                        ...
                        log_ap_prob[k - 1, a] = _log_add_lookup(
                            log_ap_prob[k - 1, a], log_add_term, log_map_vec, delta_step
                        )
    ...
    eps = 1e-300
    llr1 = np.log(np.maximum(bit_prob1[1, j1], eps) / np.maximum(bit_prob1[0, j1], eps))
    llr2 = np.log(np.maximum(bit_prob2[1, j2], eps) / np.maximum(bit_prob2[0, j2], eps))
